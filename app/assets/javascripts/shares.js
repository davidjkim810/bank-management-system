$(document).on('turbolinks:load', function(){
  ...
  function stockBrowser(){
    $('#stock_browser').on('click', function(e){
      ...
      let $stockId =  $('#stock_browser')[0].dataset.share_id;
      ...
      if (shareIndex <= userShareLength - 1) {
        $.get('/users/' + this.dataset.user_id + '/shares/' + this.dataset.share_id, function(data){

          $('#stock_browser')[0].dataset.share_index = shareIndex;
          $stockId = $('#stock_browser')[0].dataset.share_id =  data.user.shares[shareIndex].id.toString();
